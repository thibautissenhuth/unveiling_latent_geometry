...
class Generator(nn.Module):
    def __init__(self, args, img_size=224, patch_size=16, in_chans=3, num_classes=10, embed_dim=384, depth=5,
        ...
        self.latent_mixture = args.latent_mixture
        if self.latent_mixture > 0:
            self.latent_mixture_embeddings = nn.Parameter(0.1*torch.randn(self.latent_mixture, args.latent_dim))

        self.simplicial_classification = args.simplicial_classification
        ...
        self.simplicial_sampling = args.simplicial_sampling
        self.n_simplicial = args.n_simplicial
        self.simplicial_threshold = args.simplicial_threshold
        self.simplicial_linear = args.simplicial_linear
        if self.simplicial_linear:
            self.predict_z = nn.Linear(args.latent_dim,self.n_simplicial)
        elif self.simplicial_sampling:
            dims = np.random.choice(args.latent_dim,size=(self.n_simplicial),replace=False)
            embeddings = torch.zeros((self.n_simplicial,args.latent_dim))
            for i in range(self.n_simplicial):
                embeddings[i,dims[i]] = 1
            embeddings = (embeddings/embeddings.norm(dim=-1, keepdim=True) * (args.latent_dim ** 0.5))
...
#             nn.init.constant_(m.weight, 1.0)
    ...
    def forward(self, z, epoch=None,rejected=False,idx=None):
        if self.simplicial_sampling:
            with torch.no_grad():
                batch_size = z.shape[0]
                z_batch = z.new_tensor([])
                idx_batch = z.new_tensor([])
                count = 0
                while True:
                    count += 1
                    ...
                    if self.simplicial_linear:
                        with torch.no_grad():
                            dot_prod = self.predict_z(z)
                    else:
                        dot_prod = torch.matmul(z_norm,self.embeddings.t())
                    proba = torch.nn.functional.softmax(dot_prod,dim=1)                    
                    proba_max,idx_max = torch.max(proba,dim=1)
                    if idx is not None:
                        z_valid = z.new_tensor([])
                        idx_valid = z.new_tensor([])
                        for jj in range(z.shape[0]):
                            if proba_max[jj]>self.simplicial_threshold and idx_max[jj]==idx:
                                z_valid = torch.cat((z_valid,z[jj].unsqueeze(0)),dim=0)
                                idx_valid = torch.cat((idx_valid,idx_max[jj].unsqueeze(0)),dim=0)
                    else:
                        if rejected:
                            z_valid = z[torch.where(proba_max < self.simplicial_threshold)]
                            idx_valid = idx_max[torch.where(proba_max < self.simplicial_threshold)]
                        else:
                            z_valid = z[torch.where(proba_max > self.simplicial_threshold)]
                            idx_valid = idx_max[torch.where(proba_max > self.simplicial_threshold)]
                    
                    z_batch = torch.cat((z_batch,z_valid),dim=0)
                    idx_batch = torch.cat((idx_batch,idx_valid),dim=0)
                    #print(z_valid.shape,z_batch.shape)
                    if z_batch.shape[0] >= batch_size:
                        z = z_batch[:batch_size]
                        idx_batch = idx_batch[:batch_size]
                        break
                    if count > 300:
        ...
        if self.latent_mixture > 0:
            latent_mixt_ids = torch.rand(z.shape[0]).to(z.device)*self.latent_mixture
            embeddings = self.latent_mixture_embeddings[latent_mixt_ids.long()]
            z = z + embeddings
        ...
        if self.simplicial_classification:
            return output, idx_batch
        else:
            return output
