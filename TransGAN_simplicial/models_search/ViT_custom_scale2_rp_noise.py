...
class Discriminator(nn.Module):
    def __init__(self, args, img_size=32, patch_size=None, in_chans=3, num_classes=1, embed_dim=None, depth=7,
        ...
        self.simplicial_classification_epoch_end = args.simplicial_classification_epoch_end
        self.simplicial_classification = args.simplicial_classification
        if self.simplicial_classification:
            self.simplicial_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
            self.simplicial_head = nn.Linear(embed_dim,args.n_simplicial)
...
#                 drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[0], norm_layer=norm_layer),
        ...
        if self.simplicial_classification:
            trunc_normal_(self.simplicial_token, std=.02)
...
#         x = SpaceToDepth(2)(x)
        ...
        if self.simplicial_classification:
            simplicial_tokens = self.simplicial_token.expand(B, -1, -1)
            x = torch.cat((simplicial_tokens, x), dim=1)
    ...
    def forward(self, x, aug=True, epoch=400):
        if self.simplicial_classification:
            if epoch < self.simplicial_classification_epoch_end:
                x_simplicial, x_classif = self.forward_features(x, aug=aug, epoch=epoch)
                x_simplicial = self.simplicial_head(x_simplicial)
                x_classif = self.head(x_classif)
                return x_classif, x_simplicial
            else:
                _, x_classif = self.forward_features(x, aug=aug, epoch=epoch)
                x_classif = self.head(x_classif)
                return x_classif, None
        else:
