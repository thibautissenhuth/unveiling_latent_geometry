...
def save_checkpoint(states, idx, output_dir,args,
                    filename='checkpoint.pth'):
    torch.save(states, os.path.join(output_dir, filename))
    if args.dataset == 'church' or args.dataset=='stl10':
        idx_start = 59
    else:
        idx_start = 199
    if idx > idx_start:
        torch.save(states, os.path.join(output_dir, 'checkpoint_'+str(idx)+'.pth'))
    #if is_best:
