...
def compute_gradient_penalty(D, real_samples, fake_samples, phi, args, epoch):
    ...
    if args.simplicial_classification:
        d_interpolates,_ = D(interpolates, epoch=epoch)
    else:
...
def compute_gradient_penalty_R0(D, real_samples, phi, args):
    """Calculates the gradient penalty loss for WGAN GP"""
    # Random weight term for interpolation between real and fake samples
    interpolates = real_samples.requires_grad_(True)
    if args.simplicial_classification:
...
def train(args, gen_net: nn.Module, dis_net: nn.Module, gen_optimizer, dis_optimizer, gen_avg_param, train_loader,
    ...
    for iter_idx, (imgs, _) in enumerate(tqdm(train_loader)):
        ...
        if args.simplicial_classification:
            real_validity, _ = dis_net(real_imgs,epoch=epoch)
        else:   
            real_validity = dis_net(real_imgs)
        if args.simplicial_classification:
            fake_imgs, idx_simplicial = gen_net(z, epoch)
        ...
        if args.simplicial_classification:
            fake_validity, fake_simplicial_preds = dis_net(fake_imgs,epoch=epoch)
        else:
        ...
        if args.loss == 'hinge':
            ...
            gradient_penalty = compute_gradient_penalty_R0(dis_net, real_imgs, args.phi, args)
            ...
            d_loss = d_real_loss + d_fake_loss + args.lambda_gp * gradient_penalty
            ...
            gradient_penalty = compute_gradient_penalty(dis_net, real_imgs, fake_imgs.detach(), args.phi, args, epoch)
        ...
        if args.simplicial_classification and epoch < args.simplicial_classification_epoch_end:
            loss_ce_simplicial = CELoss(fake_simplicial_preds,idx_simplicial.long())
            d_loss = d_loss + args.simplicial_classification * loss_ce_simplicial
        ...
        if global_steps % (args.n_critic * args.accumulated_times) == 0:
            ...
            for accumulated_idx in range(args.g_accumulated_times):
                ...
                if args.simplicial_classification:
                    gen_imgs, idx_simplicial = gen_net(gen_z, epoch)
                else:
                    gen_imgs = gen_net(gen_z, epoch)
                
                if args.simplicial_classification:
                    fake_validity,fake_simplicial_preds = dis_net(gen_imgs,epoch=epoch)
                else:
                ...
                if args.simplicial_classification and epoch < args.simplicial_classification_epoch_end:
                    loss_ce_simplicial = CELoss(fake_simplicial_preds,idx_simplicial.long())
                    g_loss = g_loss + args.simplicial_classification * loss_ce_simplicial
...
#             save_image(sample_imgs, f'sampled_images_{args.exp_name}.jpg', nrow=4, normalize=True, scale_each=True)
            ...
            if args.simplicial_classification and epoch < args.simplicial_classification_epoch_end:
                tqdm.write(
                    "[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f] [ema: %f] [Classif Loss: %f] [gp: %f]" %
                    (epoch, args.max_epoch, iter_idx % len(train_loader), len(train_loader), d_loss.item(), g_loss.item(), ema_beta,loss_ce_simplicial.item(), gradient_penalty.item()))
            else:
                tqdm.write(
                    "[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f] [ema: %f] [gp: %f] " %
                    (epoch, args.max_epoch, iter_idx % len(train_loader), len(train_loader), d_loss.item(), g_loss.item(), ema_beta,gradient_penalty.item()))
            del gen_imgs
            del real_imgs
            del fake_validity
            del real_validity
...
#     mean, std = 0, 0
    ...
    if args.rank == 0: #and fid_stat is not None:
...
def save_samples(args, fixed_z, fid_stat, epoch, gen_net: nn.Module, writer_dict, clean_dir=True):
    ...
    with torch.no_grad():
        ...
        for i in range(fixed_z.size(0)):
            if args.simplicial_classification:
                sample_img,_ = gen_net(fixed_z[i:(i+1)], epoch)
            else:
    ...
    print(f'./samples/{args.exp_name}/sampled_images_{epoch}.png')
    return 0
